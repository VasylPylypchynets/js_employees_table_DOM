...
let rowsArr = Array.from(rows).map((row)=>{
    const person = {};
    for(let j = 0; j < row.children.length; j++)person[heads[j].textContent.trim()] = row.children[j].textContent.trim();
    return person;
});
...
function sortData(property, isNumeric = false) {
    ...
    sortedRows = [
        ...
        const val1 = isNumeric ? parseFloat(a[property].replace(/[$,]/g, "")) : a[property];
        const val2 = isNumeric ? parseFloat(b[property].replace(/[$,]/g, "")) : b[property];
        if (isNumeric) return isAscending ? val1 - val2 : val2 - val1;
...
const positionInput = createLabeledInput("Position:", "text", "position", "position");
